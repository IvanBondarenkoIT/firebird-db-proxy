...
class QueryRequest(BaseModel):
    ...
    query: str = Field(
        ..., description="SQL запрос (только SELECT или WITH)", min_length=1, max_length=10000
    )
    params: Optional[List[Any]] = Field(default=None, description="Параметры запроса (позиционные)")
    ...
    class Config:
        json_schema_extra = {
            "example": {"query": "SELECT ID, NAME FROM STORGRP WHERE ID = ?", "params": [1]}
        }
...
class QueryResponse(BaseModel):
    ...
    data: Optional[List[Dict[str, Any]]] = Field(
        default=None, description="Результаты запроса (массив объектов)"
    )
    rows_count: Optional[int] = Field(default=None, description="Количество возвращенных строк")
    execution_time: Optional[float] = Field(
        default=None, description="Время выполнения запроса в секундах"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "success": True,
                "data": [{"ID": 1, "NAME": "Магазин 1"}, {"ID": 2, "NAME": "Магазин 2"}],
                "rows_count": 2,
...
class HealthResponse(BaseModel):
    ...
    uptime_seconds: Optional[float] = Field(
        default=None, description="Время работы сервера в секундах"
    )
...
class ColumnInfo(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"name": "ID", "type": "INTEGER", "nullable": False}}
